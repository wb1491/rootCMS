...
class Content {
    ...
    public function where($attr) {
        ...
        if (isset($attr['where']) && $attr['where']) {
            ...
            $tmpwhere = parseWhere($attr['where']);
            foreach($tmpwhere[0] as $v){
                $where[$v[0]] = [$v[1],$v[2]];
            }
            foreach($tmpwhere[1] as $v){
        ...
        if (isset($attr['catid']) && (int) $attr['catid']) {
            ...
            $catids_str = $catinfo['arrchildid'];
            if ( $catinfo['child'] && strpos($catids_str,',') != false ) {
    ...
    public function relation($data) {
        ...
        $cache = (int) $data['cache'];
        $cacheID = to_guid_string($data);
        if ($cache && $key_array = cache($cacheID)) {
            return $key_array;
        }
        $getLastSql = array();
        $catid = intval($data['catid']);
        ...
        if ($data['relation']) {
            ...
            if (strpos($data['relation'], ',')) {
                ...
                foreach ($newRela as $modelid => $catidList) {
                    ...
                    $_list = app\content\model\Content::getInstance($modelid)->relation($data['moreinfo'] ? true : false)->where($where)->order($order)->select();
                    if (!empty($_list)) {
                        $_key_array = array_merge($_key_array, $_list);
                    }
                    $getLastSql[] = app\content\model\Content::getInstance($modelid)->getLastSql();
                }
            } else {
                $relations = explode('|', $data['relation']);
    ...
    public function category($data) {
        ...
        if (isset($data['catid'])) {
            $where['ismenu'] = ["eq",1];
            $where['parentid'] = ["eq", $data['catid']];
        }
        //设置SQL where 部分
        if (isset($data['where']) && $data['where']) {
            //$where = " type{gt}1 and status{eq}99 or field {let}1";
