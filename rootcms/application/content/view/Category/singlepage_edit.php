          ...
          <table width="100%" class="table_form ">
            <tr>
              ...
              <td><select name="setting[page_template]" id="page_template">
                  <option value="page<?php echo config("TMPL_TEMPLATE_SUFFIX")?>" selected>默认内容页：page<?php echo config("TMPL_TEMPLATE_SUFFIX")?></option>
                  <volist name="tp_page" id="vo">
                    <option value="{$vo}" <if condition="$data['setting']['page_template'] eq $vo ">selected</if>>{$vo}</option>
                  </volist>
                </select>
                <span class="gray">新增模板以page_x<?php echo config("TMPL_TEMPLATE_SUFFIX")?>形式</span></td>
            </tr>
          </table>
        </div>
      </div>
              ...
              <td><div class="user_group J_check_wrap">
                  ...
                  <volist name="Role_group" id="vo">
                    ...
                    <dd>
                      ...
                      <label><input class="J_check" type="checkbox" data-yid="J_check_priv_roleid{$vo.id}" name="priv_roleid[]" <if condition=" $vo['id'] eq 1 "> disabled</if>     <?php echo  model("Content/CategoryPriv")->check_category_priv($privs,'move',$vo['id'],1);?> value="move,{$vo.id}" ><span>移动</span></label>
                    </dd>
                   </volist>
                  </dl>
                </div></td>
