          ...
          <tr>
			...
			if(\Libs\System\RBAC::authenticate('disabled')){
				if(in_array($vo['field'],$forbid_fields)){
					...
					if($vo['disabled'] == 0){
						$operate[] = '<a href="'. url("Field/disabled",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid'],"disabled"=>0)).'">隐藏</a>';
					}else{
						$operate[] = '<a href="'. url("Field/disabled",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid'],"disabled"=>1)).'"><font color="#FF0000">显示</font></a>';
					}
				}
			}
			if(\Libs\System\RBAC::authenticate('delete')){
				if(in_array($vo['field'],$forbid_delete)){
					...
					$operate[] = '<a class="J_ajax_del" href="'. url("Field/delete",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid'])).'">删除</a>';
				}
			}
