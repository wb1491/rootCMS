      ...
      <table width="100%">
        ...
        <volist name="data" id="vo">
          <tr>
            ...
            <?php
			$op = array();
			$op[] = '<a href="javascript:;;" onClick="javascript:openwinx(\''. url("Content/edit",array("catid"=>$vo['catid'],"id"=>$vo['id'])).'\',\'\')">修改</a>';
			$op[] = '<a href="'. url("Content/delete",array("catid"=>$vo['catid'],"id"=>$vo['id'])).'" class="J_ajax_del" >删除</a>';
			if(isModuleInstall('Comments')){
				$op[] = '<a href="'. url('Comments/Comments/index',array('searchtype'=>2,'keyword'=>'c-'.$vo['catid'].'-'.$vo['id'].'')).'" target="_blank">评论</a>';
			}
			echo implode(' | ',$op);
			?>
            </td>
