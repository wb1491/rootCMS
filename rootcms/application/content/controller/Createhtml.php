...
class CreatehtmlController extends AdminBase {
    ...
    public function index() {
        if (IS_POST) {
            ...
            $this->Html->index();
            $this->success('首页更新成功！',  url('Content/Createhtml/index'));
        } else {
            $this->display();
        }
    }
    ...
    public function category() {
        if (isset($_POST['dosubmit'])) {
            ...
            if (!isset($set_catid)) {
                ...
                //缓存需要生成的栏目ID集合
                F('update_html_catid' . \app\admin\service\User::getInstance()->id, $update_url_catids);
                $message = "开始更新栏目页 ...";
                $forward =  url('Content/Createhtml/category', "set_catid=1&pagesize={$pagesize}&dosubmit=1&modelid={$modelid}&referer={$referer}");
                $this->assign("waitSecond", 200);
                $this->success($message, $forward);
                exit;
            }
            //从缓存中取得需要生成的栏目ID集合
            $catid_arr = F('update_html_catid' . \app\admin\service\User::getInstance()->id);
            //当前更新到的栏目
            ...
            if (!isset($catid_arr[$autoid])) {
                if (!empty($referer) && getCategory($catid_arr[0], 'type') != 1) {
                    ...
                    $this->success('更新完成！',  url('Content/Createhtml/category'));
                    exit;
                }
            }
            ...
            if ($page <= $total_number) {
                ...
                $forward =  url('Content/Createhtml/category', "set_catid=1&pagesize={$pagesize}&dosubmit=1&autoid={$autoid}&page={$page}&total_number={$total_number}&modelid={$modelid}&referer={$referer}");
                $this->assign('waitSecond', 200);
                $this->success($message, $forward);
                exit;
            } else {
                ...
                $forward =  url('Content/Createhtml/category', "set_catid=1&pagesize={$pagesize}&dosubmit=1&autoid={$autoid}&modelid={$modelid}&referer={$referer}");
                $this->assign('waitSecond', 200);
                $this->success($message, $forward);
                exit;
            }
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            if ($modelid) {
                ...
                if (!isset($first) && is_array($catids) && $catids[0] > 0) {
                    //把选择需要生成的栏目id写入缓存
                    F("url_show_" . \app\admin\service\User::getInstance()->id, $catids);
                    $catids = implode(',', $catids);
                    $where['catid'] = array("IN", $catids);
                    $first = 1;
                } elseif ($first) {
                    //获取缓存
                    $catids = F("url_show_" . \app\admin\service\User::getInstance()->id);
                    $catids = implode(',', $catids);
                    $where['catid'] = array("IN", $catids);
                } else {
                    $first = 0;
                ...
                if ($pages > $page) {
                    ...
                    $forward = $start ?  url("Content/Createhtml/update_urls", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&modelid={$modelid}") : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page={$page}&pages={$pages}&total={$total}", $http_url);

                    $this->assign("waitSecond", 200);
                    $this->success($message, $forward);
                } else {
                    //删除缓存
                    F("url_show_" . \app\admin\service\User::getInstance()->id, NULL);
                    $this->success("更新完成！ ...",  url("Content/Createhtml/update_urls"));
                }
            } else {
                //当没有选择模型时，需要按照栏目来更新
                if (!isset($set_catid)) {
                    ...
                    //生成需要更新生成的栏目ID缓存
                    F("update_url_catid" . \app\admin\service\User::getInstance()->id, $update_url_catids);
                    $this->assign("waitSecond", 200);
                    $this->success("开始更新 ...",  url("Content/Createhtml/update_urls", "set_catid=1&pagesize={$pagesize}&dosubmit=1"));
                    exit;
                }
                $catid_arr = F("update_url_catid" . \app\admin\service\User::getInstance()->id);
                $autoid = $autoid ? intval($autoid) : 0;
                if (!isset($catid_arr[$autoid])) {
                    $this->success("更新完成！ ...",  url("Content/Createhtml/update_urls"));
                    exit;
                ...
                if ($pages > $page) {
                    ...
                    $forward = $start ?  url("Content/Createhtml/update_urls", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1") : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page={$page}&pages={$pages}&total={$total}", $http_url);
                    $this->assign("waitSecond", 200);
                    $this->success($message, $forward);
                } else {
                    $autoid++;
                    $forward =  url("Content/Createhtml/update_urls", "set_catid=1&pagesize={$pagesize}&dosubmit=1&autoid={$autoid}");
                    $this->assign("waitSecond", 200);
    ...
    public function update_show() {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($modelid) {
                ...
                if (!isset($first) && is_array($catids) && $catids[0] > 0) {
                    //把选择需要生成的栏目id写入缓存
                    F("html_show_" . \app\admin\service\User::getInstance()->id, $catids);
                    $catids = implode(',', $catids);
                    $where['catid'] = array("IN", $catids);
                    $first = 1;
                    ...
                    //获取缓存
                    $catids = F("html_show_" . \app\admin\service\User::getInstance()->id);
                    $catids = implode(',', $catids);
                    $where['catid'] = array("IN", $catids);
                } else {
                    $first = 0;
                ...
                if (count($catids) == 1 && $catids[0] == 0) {
                    $this->assign("waitSecond", 200);
                    $this->success("更新完成...",  url("Content/Createhtml/update_show"));
                    exit;
                }
                ...
                if ($pages > $page) {
                    ...
                    $percent = round($creatednum / $total, 2) * 100;
                    $message = "共需更新 <font color=\"red\">{$total}</font> 条信息 - 已完成 <font color=\"red\">{$creatednum}</font> 条（<font color=\"red\">{$percent}%</font>）";
                    $forward = $start ?  url("Content/Createhtml/update_show", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&modelid={$modelid}") : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page={$page}&pages={$pages}&total={$total}", $http_url);
                    $this->assign("waitSecond", 200);
                    $this->success($message, $forward);
                } else {
                    //删除缓存
                    F("html_show_" . \app\admin\service\User::getInstance()->id, NULL);
                    $this->success("更新完成！ ...",  url("Content/Createhtml/update_show"));
                }
            } else {
                //当没有选择模型时，需要按照栏目来更新
                if (!isset($set_catid)) {
                    ...
                    F("update_html_catid" . \app\admin\service\User::getInstance()->id, $update_url_catids);
                    $this->assign("waitSecond", 200);
                    $this->success("开始更新 ...",  url("Content/Createhtml/update_show", "set_catid=1&pagesize={$pagesize}&dosubmit=1"));
                    exit;
                }
                ...
                if ($pages > $page) {
                    ...
                    $percent = round($creatednum / $total, 2) * 100;
                    $message = "【" . getCategory($catid, 'catname') . "】 有 <font color=\"red\">{$total}</font> 条信息 - 已完成 <font color=\"red\">{$creatednum}</font> 条（<font color=\"red\">{$percent}%</font>）";
                    $forward = $start ?  url("Content/Createhtml/update_show", "type={$type}&dosubmit=1&first={$first}&fromid={$fromid}&toid={$toid}&fromdate={$fromdate}&todate={$todate}&pagesize={$pagesize}&page={$page}&pages={$pages}&total={$total}&autoid={$autoid}&set_catid=1") : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page={$page}&pages={$pages}&total={$total}", $http_url);
                    $this->assign("waitSecond", 200);
                    $this->success($message, $forward);
                } else {
                    $autoid++;
                    $forward =  url("Content/Createhtml/update_show", "set_catid=1&pagesize={$pagesize}&dosubmit=1&autoid={$autoid}");
                    $this->assign("waitSecond", 200);
                    $this->success("开始更新 .." . getCategory($catid, 'catname') . " ...", $forward);
                }
            }
