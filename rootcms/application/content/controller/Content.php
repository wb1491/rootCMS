...
class ContentController extends AdminBase {
    ...
    protected function _initialize() {
        parent::_initialize();
        $this->catid = input('request.catid', $_POST['info']['catid'], 'intval');
    ...
    public function public_categorys() {
        ...
        foreach ($categorys as $rs) {
            ...
            if ($rs['child'] == 0) {
                $data['target'] = 'right';
                $data['url'] =  url('Content/classlist', array('catid' => $rs['catid']));
                //设置图标 
            ...
            if ($rs['type'] == 1 && $rs['child'] == 0) {
                $data['url'] =  url('Content/add', array('catid' => $rs['catid']));
                //设置图标 
    ...
    public function classlist() {
        ...
        if (empty($catInfo)) {
            $this->error('该栏目不存在！',  url('admin/Main/index'));
        }
        //查询条件
        $where = array();
        ...
        if (!empty($search)) {
            ...
            //推荐
            $posids = input('get.posids', 0, 'intval');
            if (!empty($posids)) {
                $where["posid"] = array("EQ", $posids);
                $this->assign("posids", $posids);
            }
            //搜索字段
            $searchtype = input('get.searchtype', null, 'intval');
            //搜索关键字
            $keyword = \Input::getVar(input('get.keyword'));
            if (!empty($keyword)) {
        ...
        if (IS_POST) {
            ...
            if ($category['type'] == 0) {
                ...
                if ($db->savePage($_POST)) {
                    //扩展字段处理
                    if ($_POST['extend']) {
                         model('Content/Category')->extendField($catid, $_POST);
                    }
            ...
            if ($category['type'] == 0) {
                ...
                $info =  model('Content/Page')->getPage($this->catid);
                if ($info && $info['style']) {
                ...
                $this->assign("category", $category);
                //栏目扩展字段
                $this->assign('extendList',  model("Content/Category")->getExtendField($this->catid));
    ...
    public function delete() {
        if (IS_POST) {
            $this->catid = input('get.catid', 0, 'intval');
            $Categorys = getCategory($this->catid);
            if (empty($Categorys)) {
                $this->error("该栏目不存在！");
            }
    ...
    public function listorder() {
        ...
        if (is_array($listorders)) {
            ...
            $this->success('更新成功！',  url('classlist', array('catid' => $this->catid)));
        } else {
            $this->error('参数错误！');
        }
    }
    ...
    public function public_imagescrop() {
        ...
        $catid = input('get.catid', $this->catid, 'intval');
        if (!$catid) {
            $this->error('栏目不存在！');
        }
        if ($picurl) {
        ...
        $module = input('get.module', MODULE_NAME);
    ...
    public function remove() {
        if (IS_POST && isset($_POST['fromtype'])) {
            ...
            //移动类型
            $fromtype = input('post.fromtype', '', 'intval');
            //需要移动的信息ID集合
            $ids = $_POST['ids'];
            //需要移动的栏目ID集合
            $fromid = $_POST['fromid'];
            //目标栏目
            $tocatid = input('post.tocatid', '', 'intval');
            if (!$tocatid) {
                $this->error("目标栏目不正确！");
            }
            switch ($fromtype) {
                ...
                case 0:
                    if ($ids) {
                        ...
                        $this->success('移动成功！',  url('Createhtml/update_urls'));
                    } else {
                ...
                case 1:
                    ...
                    if ($model->where($where)->save(array('catid' => $tocatid, 'url' => ''))) {
                        $this->success('移动成功，请使用《批量更新URL》更新新的地址！！',  url('Createhtml/update_urls'));
                    } else {
                        $this->error('移动失败！');
                    }
                    break;
            ...
            switch ($action) {
                ...
                case "position_list":
                    ...
                    if ($posid && is_array($posid)) {
                        $position_data_db =  model('Content/Position');
