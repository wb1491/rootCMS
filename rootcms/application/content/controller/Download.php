...
class DownloadController extends Base {
    ...
    public function index() {
        ...
        if (!empty($data)) {
            ...
            //如果是视频模型，直接跳转到视频地址
            if($modelid == 4){
                redirect( url("Download/d", array('key' => urlencode($key))));
            }
            
            $this->assign("info", $data);
            $this->assign("fileurl",  url("Download/d", array('key' => urlencode($key))));
    ...
    public function d() {
        ...
        if ($aut == $key[0] && $setting) {
            //取得下载字段内容
            $downfiles = M($tablename)->where(array("id" => $this->id))->column($f);
            $dowUnserialize = unserialize($downfiles);
            //判断是否可以反序列化
            if ($dowUnserialize) {
                $info = $dowUnserialize[$k];
                ...
                if ((int) $info['groupid'] > 0 || (int) $info['point'] > 0) {
                    if (!$this->userid) {
                        $this->error("请登陆后再下载！",  url("member/Index/login", "forward=" . urlencode(get_url())));
                    }
            ...
            if (file_exists($fileurl)) {
                if($modelid == 4 && !is_mobile() ){
                    $this->putMovie($fileurl);
                }else{
    ...
    private function putMovie($file) {
        ...
        header("Content-Length: " . filesize($file) );
        header("Content-Disposition: filename=" . siconv( basename($file),"utf-8"));
        readfile($file);
    }
