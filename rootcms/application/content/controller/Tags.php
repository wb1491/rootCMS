...
class TagsController extends AdminBase {
        ...
        if (IS_POST) {
            ...
            if (is_array($listorder)) {
                foreach ($listorder as $tagid => $v) {
                     model('Content/Tags')->where(array('tagid' => $tagid))->save(array('listorder' => (int) $v));
                }
    ...
    public function create() {
        if (IS_POST || isset($_GET['modelid'])) {
            $modelid = input('request.modelid', 0, 'intval');
            $_GET['modelid'] = $modelid;
            $lun = input('get.lun', 0, 'intval'); //第几轮 0=>1
            ...
            if (isset($_GET['delete'])) {
                //清空
                $db->execute('TRUNCATE TABLE  `' . config("DB_PREFIX") . 'tags_content`');
                $tagdb->execute('TRUNCATE TABLE  `' . config("DB_PREFIX") . 'tags`');
            }
            unset($_GET['delete']);
            $model = cache('Model');
            if ((int) $_GET['mo'] == 1) {
    ...
    protected function createUP($models_v, $firstRow, $mlun) {
        $db = M('TagsContent');
        $tagdb = M('Tags');
        $keywords = M(ucwords($models_v['tablename']))->where(array("status" => 99))->order(array("id" => "ASC"))->limit("{$firstRow},{$mlun}")->column("id,id,keywords,tags,url,title,catid,updatetime");
        foreach ($keywords as $keyword) {
            ...
            foreach ($key as $key_v) {
                ...
                $key_v = trim($key_v);
                if ($tagdb->where(array('tag' => $key_v))->column('tagid')) {
