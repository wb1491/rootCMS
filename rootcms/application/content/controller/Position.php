...
class PositionController extends AdminBase {
    ...
    public function rebuilding() {
        $action = input('get.action', 0, 'intval');
        ...
        if ($action == 0) {//初始化
            ...
            $lun = input('get.lun', 1, 'intval');
            $i = input('get.i', 0, 'intval');
    ...
    public function item() {
        if (IS_POST) {
            ...
            foreach ($data as $k => $v) {
                ...
                $data[$k]['data']['url'] = M($tab)->where(array("id" => $v['id']))->column("url");
            }
    ...
    public function public_category_load() {
        ...
        echo \Form::checkbox($categorys, input('get.catid', 0, ''), 'name="info[catid][]"', '', 1);
    }
