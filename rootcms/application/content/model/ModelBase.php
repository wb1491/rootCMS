...
class ModelBase extends Model {
    ...
    public function deleteModel($modelid) {
        ...
        cache("Model", NULL);
        //删除所有和这个模型相关的字段
         model("ModelField")->where(array("modelid" => $modelid))->delete();
        //删除主表
    ...
    public function sql_split($sql) {
        if (mysql_get_server_info() > '4.1' && config('DB_CHARSET')) {
            $sql = preg_replace("/TYPE=(InnoDB|MyISAM|MEMORY)( DEFAULT CHARSET=[^; ]+)?/", "ENGINE=\\1 DEFAULT CHARSET=" . config('DB_CHARSET'), $sql);
        }
        if (C("DB_PREFIX") != "shuipfcms_") {
            $sql = str_replace("shuipfcms_", config("DB_PREFIX"), $sql);
        }
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
