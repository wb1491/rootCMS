...
class Category extends Model {
    ...
    protected function extendFieldDel($where) {
        ...
        return db('CategoryField')->where($where)->delete() !== false ? true : false;
    }
    ...
    public function extendField($catid, $post) {
        ...
        if (!empty($post['extenddelete'])) {
            ...
            db('CategoryField')->where(array('fid' => array('IN', $extenddelete)))->delete();
        }
        //查询出该栏目扩展字段列表
        $extendFieldLisr = array();
        foreach (db('CategoryField')->where(array('catid' => $catid))->field('fieldname')->select() as $rs) {
        ...
        if (!empty($post['extend_config']) && is_array($post['extend_config'])) {
            ...
            foreach ($post['extend_config'] as $field => $rs) {
                ...
                $rs['catid'] = $catid;
                $data = db('CategoryField')->validate($validate)->create($rs);
                if ($data) {
                    ...
                    $fieldId = db('CategoryField')->add($data);
                    if ($fieldId) {
