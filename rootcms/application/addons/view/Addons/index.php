...
<div class="wrap J_check_wrap">
  ...
  <div class="prompt_text">
    <ul>
      ...
      <li><font color="#FF0000">操作插件需谨慎，有被清空数据库的危险！</font></li>
    </ul>
  </div>
    ...
    <table width="100%">
      ...
      <tr>
        ...
        <td align="center">
          ...
          <a  href="{:url('Addons/install', array('addonename'=>$vo['name'])  )}" class="btn btn_submit btn_big">安装</a>
          <else/>
          <if condition=" $vo['config'] ">
          <a  href="{:url('Addons/config', array('id'=>$vo['id'])  )}" class="btn btn_submit btn_big">设置</a>
          </if>
          <if condition=" $vo['status'] ">
          <a  href="{:url('Addons/status', array('id'=>$vo['id'])  )}" class="btn btn_big">禁用</a>
          <else/>
          <a  href="{:url('Addons/status', array('id'=>$vo['id'])  )}" class="btn btn_big">启用</a>
          </if>
          <a  href="{:url('Addons/uninstall', array('id'=>$vo['id'])  )}" class="btn btn_big">卸载</a>
          </if>
          <a  href="{:url('Addons/unpack', array('addonname'=>$vo['name'])  )}" class="btn btn_big">打包</a>
         </td>
      </tr>
      </volist>
    </table>
