...
class Addons extends Model {
    ...
    public function upgradeAddon($addonName) {
        ...
        //判断是否有数据库升级脚本
        if (file_exists($exec)) {
            $sql = $this->sqlSplit(file_get_contents($exec), config("DB_PREFIX"));
    ...
    public function delAddonMenu($info) {
        ...
        //查询出“插件后台列表”菜单ID
        $menuId = M("Menu")->where(array("app" => "Addons", "controller" => "Addons", "action" => "addonadmin"))->column('id');
        if (empty($menuId)) {
            return false;
        }
        //删除对应菜单
    ...
    protected function addonBehavior($addonName) {
        ...
        //取得已经存在的行为列表
        $behavior = M('Behavior')->column('name', true);
        //交集数组
