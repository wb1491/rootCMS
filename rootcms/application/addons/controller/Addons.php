...
class Addons extends AdminBase {
    ...
    public function index() {
        ...
        if (!empty($addons)) {
            ...
            foreach ($addons as $key => $rs) {
                ...
                if (file_exists($path)) {
                    $addons[$key]['url'] =  url("Addons/{$rs['name']}/index");
                } else {
                    $addons[$key]['url'] = '';
                }
            }
    ...
    public function install() {
        //插件名称
        $addonName = trim(input('get.addonename'));
        if (empty($addonName)) {
    ...
    public function config() {
        if (IS_POST) {
            ...
            if (false !== $this->addons->where(array('id' => $id))->save(array('config' => serialize($config)))) {
                ...
                $this->success('保存成功！',  url('Addons/index'));
            } else {
                $this->error('保存失败！');
            }
        } else {
    ...
    public function status() {
        ...
        if ($this->addons->statusAddon($addonId)) {
            $this->success('插件状态成功！',  url('Addons/index'));
        } else {
            $error = $this->addons->getError();
...
";
            ...
            //查询出系统可用行为
            $behaviorList = M('Behavior')->where(array('status' => 1))->column('name,title', true);
