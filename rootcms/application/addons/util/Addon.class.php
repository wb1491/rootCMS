...
abstract class Addon {
    ...
    public function getAddonConfig($name = NULL) {
        ...
        $config = M('Addons')->where($where)->column('config');
        if ($config) {
            //反序列化
    ...
    protected function renderFile($templateFile = '') {
        if (empty($templateFile)) {
            //如果没有填写，尝试直接以当前插件名称
            $templateFile = $this->addonPath . 'View/Behavior/' . strtolower($this->addonName) . config('TMPL_TEMPLATE_SUFFIX');
        } else {
            $templateFile = $this->addonPath . 'View/Behavior/' . $templateFile . config('TMPL_TEMPLATE_SUFFIX');
        }
        //检查模板
    ...
    protected function checkCache($tmplTemplateFile) {
        ...
        if (!is_file($tmplCacheFile)) {
            ...
            // 模板文件如果有更新则缓存需要更新
            return false;
        } elseif (C('TMPL_CACHE_TIME') != 0 && time() > filemtime($tmplCacheFile) + config('TMPL_CACHE_TIME')) {
            // 缓存是否在有效期
            return false;
        }
        // 缓存有效
