...
class CommentsController extends AdminBase {
    ...
    protected function _initialize() {
        ...
        $this->db =  model('Comments/Comments');
    }
    ...
    public function index() {
        if (IS_POST) {
            ...
            $searchtype = input('get.searchtype', 0, 'intval');
            $type = array(
                0 => "content", //评论内容
                1 => "author", //评论作者
                2 => "comment_id"//所属文章id
    ...
    public function edit() {
        if (IS_POST) {
            ...
            if (false !== $this->db->editComments($post)) {
                $this->success('评论更新成功！',  url('Comments/index'));
            } else {
                $this->error($this->db->getError());
            }
        } else {
    ...
    public function replycomment() {
        if (IS_POST) {
            ...
            $catid = input('post.comment_catid', 0, 'intval');
            $id = input('post.comment_id', 0, 'intval');
            ...
            if (false !== $commentsId) {
                if ($commentsId === -1) {
                    ...
                    $this->success("评论发表成功！",  url('Comments/Comments/index'));
                }
            } else {
                $this->error($this->db->getError());
            }
            ...
            if ($r) {
                ...
                $this->assign("author_email", \app\admin\service\User::getInstance()->email);
                $this->assign("author", \app\admin\service\User::getInstance()->username);
                $this->assign("data", $data);
    ...
    public function config() {
        ...
        if (IS_POST) {
            ...
            if ($db->where($where)->save($data) !== false) {
                ...
                $this->success("更新成功！",  url("Comments/Comments/config"));
            } else {
                $this->error("更新失败！",  url("Comments/Comments/config"));
            }
        } else {
            $data = $db->find();
            $this->assign("data", $data);
    ...
    public function fenbiao() {
        ...
        for ($i = 1; $i <= $stbsum; $i++) {
            ...
            $data[] = array(
                "id" => $i,
                "count" => $d->count(),
                "tablename" => config("DB_PREFIX") . "comments_data_" . $i
            );
        }
        $this->assign("data", $data);
        $this->assign("r", $r);
    ...
    public function addfenbiao() {
        if ( model("Comments/CommentsField")->addfenbiao()) {
            $this->success("分表创建成功！");
        } else {
            $this->error("创建分表失败！");
        }
