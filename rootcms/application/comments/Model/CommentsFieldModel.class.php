...
class CommentsFieldModel extends Model {
    ...
    public function addfenbiao() {
        //取得表结构
        $CREATE = $this->query("SHOW CREATE TABLE `" . config("DB_PREFIX") . "comments_data_1`;");
        ...
        $create = str_replace("" . config("DB_PREFIX") . "comments_data_1", "" . config("DB_PREFIX") . "comments_data_" . ((int) $r['stbsum'] + 1), $create);
        if (empty($create)) {
            return false;
        }
        //执行SQL
    ...
    public function fieldDelete($fid) {
        ...
        if ((int) $r['issystem'] == 1) {
            $this->query("alter table " . config("DB_PREFIX") . "comments DROP `" . $r['f'] . "`");
        } else {
            for ($i = 1; $i <= (int) $Setting['stbsum']; $i++) {
                $this->query("alter table " . config("DB_PREFIX") . "comments_data_" . $i . " DROP `" . $r['f'] . "` ");
            }
        }
    ...
    public function fieldAdd($data) {
        ...
        if ($id) {
            if ((int) $data['issystem']) {
                //对主表添加字段
                $this->query("alter table " . config("DB_PREFIX") . "comments add " . $field);
            } else {
                //所有副表都增加字段
                for ($i = 1; $i <= (int) $setting['stbsum']; $i++) {
                    $this->query("alter table " . config("DB_PREFIX") . "comments_data_" . $i . " add " . $field);
                }
            }
            return $id;
        } else {
    ...
    public function fieldEdit($data) {
        ...
        if (false !== $id) {
            if ((int) $r['issystem']) {
                //对主表添加字段
                $this->query("alter table " . config("DB_PREFIX") . "comments change `" . $r['f'] . "` " . $field);
            } else {
                //所有副表都增加字段
                for ($i = 1; $i <= (int) $setting['stbsum']; $i++) {
                    $this->query("alter table " . config("DB_PREFIX") . "comments_data_" . $i . " change `" . $r['f'] . "` " . $field);
                }
            }
            return $id;
        } else {
