...
class LinksController extends AdminBase {
    ...
    public function index() {
        //分类id
        $termsid = input('get.termsid', 0, 'intval');
        $where = array();
        if ($termsid > 0) {
        ...
        $Termsdb = M("Terms");
        $Terms = $Termsdb->where(array("module" => "links"))->column("id,name");
    ...
    public function termsedit() {
        ...
        if (IS_POST) {
            ...
            $this->assign($info);
            $this->assign("big_menu", array( url("Links/termsadd"), "添加分类"));
            $this->display();
        }
    }
