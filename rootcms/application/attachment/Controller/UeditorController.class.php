...
class UeditorController extends AttachmentsController {
    ...
    protected function _initialize() {
        ...
        if (IS_POST) {
            ...
            $sess_id = input('get.sessid', 0);
            $userid = input('get.uid');
            $this->isadmin = input('get.isadmin', 0, 'intval');
            $key = md5(C("AUTHCODE") . $sess_id . $userid . $this->isadmin);
            if ($key != $authkey) {
                exit(json_encode(array('state' => '身份认证失败！')));
            } else {
                $this->uid = $userid;
                $this->groupid = input('get.groupid');
            }
        }
        parent::_initialize();
        if ($this->isadmin) {
    ...
    public function run() {
        ...
        switch ($action) {
            ...
            case 'uploadscrawl':
                ...
                $module = input('get.module', ($catid ? 'content' : MODULE_NAME), 'trim');
                ...
                $fileSize = strlen($img);
                //上传目录
                $savePath =  model('Attachment/Attachment')->getFilePath($module, 'Y/m', time());
                $up = new \UploadFile();
                //保存文件名
