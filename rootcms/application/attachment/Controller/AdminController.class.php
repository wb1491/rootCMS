...
class AdminController extends AdminBase {
    ...
    public function swfupload() {
        if (IS_POST) {
            $sess_id = input("post.sessid", 0);
            $key = md5(C("AUTHCODE") . $sess_id . self::isadmin);
            //密钥验证
            if (input("post.swf_auth_key") != $key) {
                exit("0,权限认证失败！" . input("post.swf_auth_key") . "|" . config("AUTHCODE"));
            }
            //回调函数
            $Callback = false;
            //用户ID
            $upuserid = input('post.uid', 0, 'intval');
            //取得栏目ID
            $catid = input('post.catid', 0, 'intval');
            //取得模块名称
            $module = input('post.module', 'content', 'trim,strtolower');
            //获取附件服务
            $Attachment = service("Attachment", array('module' => $module, 'catid' => $catid, 'isadmin' => self::isadmin, 'userid' => $upuserid));
            //缩略图宽度
            $thumb_width = input('post.thumb_width', 0, 'intval');
            $thumb_height = input('post.thumb_height', 0, 'intval');
            ...
            if (input('post.watermark_enable', 0, 'intval')) {
                $Callback = array(
    ...
    public function public_album_load() {
        ...
        $where = array();
        $db = M("Attachment");
        $filename = input('get.filename', '', 'base64_decode,urldecode');
        $args = input('get.args', '', '');
        $args = explode(",", $args);
        empty($filename) ? : $where['filename'] = array('like', '%' . $filename . '%');
        $uploadtime = input('get.uploadtime', '', '');
        if (!empty($uploadtime)) {
