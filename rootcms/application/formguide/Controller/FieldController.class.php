...
class FieldController extends AdminBase {
    ...
    protected function _initialize() {
        parent::_initialize();
        $this->formid = input('get.formid', 0, 'intval');
        //字段类型存放目录
        $this->fields =  model('Content/ModelField')->getFieldPath();
        //允许使用的字段列表
        $this->banfie = array("text", "textarea", "box", "number", "editor", "datetime", "downfiles", "image", "images", "omnipotent");
        $this->modelfield =  model("Formguide/FormaguideField");
    }
    ...
    public function add() {
        if (IS_POST) {
            ...
            $modelid = input('get.formid', 0, 'intval');
            if (!$modelid) {
    ...
    public function edit() {
        if (IS_POST) {
            ...
            $modelid = input('post.modelid', 0, 'intval');
            //字段ID
            $fieldid = input('post.fieldid', 0, 'intval');
            if (empty($modelid)) {
                $this->error('模型ID不能为空！');
            }
            if (empty($fieldid)) {
            ...
            if ($this->modelfield->editField($post, $fieldid)) {
                cache('Model_form',NULL);
                $this->success("更新成功！",  url("Field/index", array("formid" => $modelid)));
            } else {
                $error = $this->modelfield->getError();
    ...
    public function delete() {
        //字段ID
        $fieldid = input('get.fieldid', 0, 'intval');
        if (empty($fieldid)) {
            $this->error('字段ID不能为空！');
        }
