...
class Index extends Base {
    ...
    public function index() {
        ...
        $seo = seo();
        $this->assign('seo', $seo);
        $q = input('request.q', '', 'trim');
        if ($q) {
            G('search');
            if (empty($q)) {
                header('Location: ' .  url("search/Index/index"));
                exit;
            }
            if (IS_POST) {
                $data = array("q" => input('request.q', '', 'trim'));
                $data = array_merge($_POST, $data);
                header('Location: ' .  url("search/Index/index", $data));
                exit;
            }
            //关键字
            //$q = \Input::forSearch(input('request.q', '', 'trim'));
            $q = htmlspecialchars(strip_tags(input('request.q', '', 'trim')));
            //时间范围
            $time = input('get.time');
            //模型
            $mid = input('get.modelid', 0, 'intval');
            //栏目
            ...
            if ($this->config['sphinxenable']) {
                ...
                //设置返回结果集偏移量和数目
                $page = input('get.' . config("VAR_PAGE"), 1, 'intval');
                ...
                if ($this->config['dzsegment']) {
                    $segment_q =  model('Search/Search')->discuzSegment($q);
                } else {
                    $segment_q =  model('Search/Search')->segment($q);
                }
