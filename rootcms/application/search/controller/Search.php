...
class Search extends AdminBase {
    ...
    public function create() {
        ...
        if (isset($_GET['start'])) {
            //每轮更新数
            $pagesize = input('get.pagesize', 0, 'intval');
            $_GET['pagesize'] = $pagesize = $pagesize > 1 ? $pagesize : 100;
            //模型
            $_GET['modelid'] = $modelid = input('get.modelid', 0, 'intval');
            //第几轮更新
            $page = $_GET['start'] = input('get.start', 0, 'intval');
            //总共几轮
            $pages = input('get.pages', 0, 'intval');
            //信息总数
            $total = input('get.total', 0, 'intval');
            $model = cache('Model');
            ...
            if ($modelid) {
                ...
                if ($pages == $page || $page > $pages) {
                    $this->success("更新完成！ ...",  url("Search/create"));
                    exit;
                }
                if ($pages > $page) {
                    $page++;
                    ...
                    $forward =  url("Search/create", $_GET);
                ...
                $autoid = input('get.autoid', 0, 'intval');
                ...
                if ($pages == $page || $page > $pages) {
                    ...
                    unset($_GET['total']);
                    $this->assign("waitSecond", 200);
                    $this->success("模型【" . $model[$modelid]['name'] . "】更新完成 ...",  url("Search/create", $_GET));
                    exit;
                }
            ...
            if (IS_POST) {
                //每轮更新数
                $pagesize = input('post.pagesize', 100, 'intval');
                //模型
                ...
                $this->success("开始进行索引重建...",  url("Search/create", array("start" => 1, "pagesize" => $pagesize, "modelid" => $modelid)));
            } else {
                $model = cache('Model');
                foreach ($model as $k => $rs) {
                    if ($rs['type']) {
