    ...
    <table width="100%">
      ...
      <tr>
        ...
        <td align="center">
          <?php
		  $op = array();
		  if(! model('Addons/Addons')->isInstall($vo['name'])){
			  $op[] = '<a href="'. url('install',array('sign'=>$vo['sign']?:$vo['name'])).'" class="btn btn_submit mr5 Js_install">安装</a>';
		  }else{
			 //有安装，检测升级
			 if($vo['upgrade']){
				 $op[] = '<a href="'. url('upgrade',array('sign'=>$vo['sign']?:$vo['name'])).'" class="btn btn_submit mr5 Js_upgrade" id="upgrade_tips_'.$vo['name'].'">升级到最新'.$vo['newVersion'].'</a>';
			 }
		  }
		  echo implode('  ',$op);
		  if($vo['price']){
