...
<div class="wrap J_check_wrap">
  ...
  <form method='post'  class="J_ajaxForm"  action="{:url('Config/extend')}">
  ...
  <div class="table_full">
    <form method='post'   id="myform" class="J_ajaxForm"  action="{:url('Config/extend')}">
      <table width="100%"  class="table_form">
        <volist name="extendList" id="vo">
        <php>$setting = unserialize($vo['setting']);</php>
        <tr>
          <th width="200">{$setting.title} <a href="{:url('Config/extend',array('fid'=>$vo['fid'],'action'=>'delete'))}" class="J_ajax_del" title="删除该项配置" style="color:#F00">X</a><span class="gray"><br/>键名：{$vo.fieldname}</span></th>
          <th class="y-bg">
          <switch name="vo.type">
             <case value="input">
