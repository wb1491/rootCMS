      ...
      <tr>
        ...
        <td align="center">
		  ...
		  if(!isModuleInstall($vo['module'])){
			  $op[] = '<a href="'. url('install',array('sign'=>$vo['sign'])).'" class="btn btn_submit mr5 Js_install">安装</a>';
		  }else{
			 //有安装，检测升级
			 if($vo['upgrade']){
				 $op[] = '<a href="'. url('upgrade',array('sign'=>$vo['sign'])).'" class="btn btn_submit mr5 Js_upgrade" id="upgrade_tips_'.$vo['sign'].'">升级到最新'.$vo['newVersion'].'</a>';
			 }
		  }
		  echo implode('  ',$op);
		  if($vo['price']){
