...
class Addonshop extends AdminBase {
    ...
    public function index() {
        ...
        if (IS_AJAX) {
            ...
            foreach ($data['data'] as $sign => $rs) {
                $version = M('Addons')->where(array('sign' => $rs['sign']))->column('version');
    ...
    public function install() {
        ...
        $this->assign('stepUrl',  url('public_step_1', array('sign' => $sign)));
    ...
    public function public_step_1() {
        ...
        $this->success('文件下载完毕！',  url('public_step_2', array('package' => $packageUrl)));
    }
    ...
    public function public_step_2() {
        ...
        $this->success('移动文件到插件目录中，等待安装！',  url('public_step_3', array('name' => $data['name'])));
    }
    ...
    public function public_step_3() {
        ...
        $name = input('get.name');
        cache('Cloud', NULL);
        if ( model('Addons/Addons')->installAddon($name)) {
            $this->success('插件安装成功！');
        } else {
            $error =  model('Addons/Addons')->getError();
            //删除目录
    ...
    public function upgrade() {
        ...
        if (false === $data) {
            ...
            $version = M('Addons')->where(array('sign' => $data['sign']))->column('version');
    ...
    public function public_upgrade_1() {
        ...
        $sign = $data['sign'];
        //检查是否安装
        if (! model('Addons/Addons')->isInstall($data['name'])) {
        ...
        $this->success('升级包文件下载完毕！',  url('public_upgrade_2', array('package' => $packageUrl)));
    }
    ...
    public function public_upgrade_2() {
        ...
        $this->success('移动文件到插件目录成功，等待升级！',  url('public_upgrade_3', array('name' => $data['name'])));
    }
    ...
    public function public_upgrade_3() {
        ...
        $name = input('get.name');
        cache('Cloud', NULL);
        if ( model('Addons/Addons')->upgradeAddon($name)) {
