...
class Rbac extends AdminBase {
    ...
    public function rolemanage() {
        ...
        $this->assign("data",  model("admin/Role")->order(array("listorder" => "asc", "id" => "desc"))->select())
                ->display();
    }
    ...
    public function roleadd() {
        if (IS_POST) {
            if ( model("admin/Role")->create()) {
                if ( model("admin/Role")->add()) {
    ...
    public function roleedit() {
        ...
        if (IS_POST) {
            if ( model("admin/Role")->create()) {
                if ( model("admin/Role")->where(array('id' => $id))->save()) {
            ...
            $data =  model("admin/Role")->where(array("id" => $id))->find();
            if (empty($data)) {
    ...
    public function authorize() {
        if (IS_POST) {
            ...
            $this->assign('json', json_encode($json))
                    ...
                    ->assign('name',  model("admin/Role")->getRoleIdName($roleid))
                    ->display();
        }
    }
    ...
    public function setting_cat_priv() {
        if (IS_POST) {
            ...
            $roleid = input('get.roleid', 0, 'intval');
            if(empty($roleid)){
