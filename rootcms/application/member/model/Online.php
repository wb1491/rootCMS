...
class OnlineModel extends Model {
    ...
    public function getOnlineUserList($page = 1, $limit = 10) {
        $count = $this->alias('as O')->join(' INNER JOIN `' . config('DB_PREFIX') . 'member` as M ON O.userid = M.userid')->count('M.userid');
        $page = page($count, $limit, $page);
        $cache = $this->alias('as O')
                ->join(' INNER JOIN `' . config('DB_PREFIX') . 'member` as M ON O.userid = M.userid')
                ->field('M.*')
    ...
    public function registerOnlineStatus($userid = 0) {
        ...
        //取得用户信息
        $userInfo =  model('Member')->getUserInfo((int) $userid, 'userid,username');
        if (empty($userInfo)) {
            $this->error = '用户不存在！';
            return false;
        }
    ...
    public function maintainOnlineStatus($userid = 0) {
        ...
        $onlineId = $this->where(array('userid' => $userid))->column('lasttime');
        if (empty($onlineId)) {
    ...
    public function getLastActiveTime($userid = 0) {
        ...
        $getLastActiveTime[$userid] = $this->where(array('userid' => (int) $userid))->column('lasttime');
        return $getLastActiveTime[$userid];
    }
