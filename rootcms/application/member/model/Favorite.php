...
class Favorite extends Model {
    ...
    public function isFavoriteCompetence($userid) {
        ...
        //取得用户配置信息
        $userConfig =  model('member/Member')->getUserConfig((int) $userid);
        if (empty($userConfig)) {
            return false;
        }
    ...
    public function favoriteAdd($data) {
        ...
        //取得信息
        $info = \app\content\model\Content::getInstance($modelid)->where(array('id' => $id, 'catid' => $catid, 'status' => 99))->field('id,catid,title,url,description,thumb')->find();
        if (empty($info)) {
            $this->error = '信息为空！';
        }
