...
class Member extends \think\Model {
    ...
    protected $_validate = array(
        ...
        //array('email', 'email', '邮箱地址有误！',2,'regex',1),
        array('username', '', '帐号名称已经存在！', 0, 'unique', 1),
        //array('nickname', '', '该昵称已经存在！', 0, 'unique', 1),
        array('mobile', '', '手机号码已经存在！', 0, 'unique', 1),
        array('pwdconfirm', 'password', '两次输入的密码不一样！', 2, 'confirm'),
        //callback
        array('username', 'checkName', '用户名已经存在或不合法！', 0, 'callback', 1),
        array('mobile', 'checkMobile', '手机号码已经存在或者输入不正确！', 0, 'callback', 1),
    ...
    public function checkEmail($email) {
        if (service("Passport")->userCheckeMail($email,false)) {
            return true;
        }
        return false;
    }
    ...
    public function checkMobile($mobile){
        if (service("Passport")->userCheckeMobile($mobile,true)) {
            return true;
        }
        return false;
    }
    ...
    public function ChangePassword($identifier, $password) {
        ...
        $verify = $this->where($term)->column('verify');
    ...
    public function getUserConfig($userid) {
        ...
        //检查缓存是否存在
        $userConfig = cache('user_config_' . $userid);
        if (!empty($userConfig)) {
            return $userConfig;
        }
        //取得用户信息
        ...
        //进行缓存
        cache('user_config_' . $userid, $getUserConfig, 3600);
        return $getUserConfig;
    }

    //会员配置缓存
    public function member_cache() {
        $data = unserialize(model('Module')->where(array('module' => 'member'))->value('setting'));
        sys_cache("Member_Config", $data);
    ...
    public function member_model_cahce() {
        $data =  model('content/Model')->getModelAll(2);
        sys_cache("Model_Member", $data);
        return $data;
    }
