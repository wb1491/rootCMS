...
<div class="wrap J_check_wrap">
  <div class="nav">
  	<div class="return"><a href="{:url('Model/index','menuid=93')}">返回模型管理</a></div>
    <ul class="cc">
      <li class="current"><a href="{:url('Field/index',array('modelid'=>$modelid)  )}">管理模型字段</a></li>
      <li ><a href="{:url('Field/add',array('modelid'=>$modelid)  )}">添加字段</a></li>
    </ul>
  </div>
  <form name="myform" action="{:url("Field/listorder")}" method="post" class="J_ajaxForm">
  ...
  <table width="100%" cellspacing="0" >
        ...
        <volist name="data" id="vo">
          <tr>
            ...
            <td align='center'>
            <a href="{:url("Field/edit",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid']))}">修改</a> | 
            <if condition=" in_array($vo['field'],$forbid_fields) ">
            <font color="#BEBEBE"> 禁用 </font>|
            <else />
                 <if condition=" $vo['disabled'] eq 0 ">
                 <a href="{:url("Field/disabled",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid'],"disabled"=>0))}">禁用</a> |
                 <else />
                 <a href="{:url("Field/disabled",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid'],"disabled"=>1))}"><font color="#FF0000">启用</font></a> |
                 </if>
            </if>
            <if condition=" in_array($vo['field'],$forbid_delete) ">
            <font color="#BEBEBE"> 删除</font>
            <else />
            <a href="javascript:confirmurl('{:url("Field/delete",array("fieldid"=>$vo['fieldid'],"modelid"=>$vo['modelid']))}','确认要删除 『 {$vo.name} 』 吗？')">删除</a>
            </if>
            </td>
          </tr>
        </volist>
