...
class Member extends AdminBase {
    ...
    protected function _initialize() {
        ...
        $tmpvip  = getVipType();
        foreach( $tmpvip as $v){
            $vipdata[$v['id']] = $v['typename'];
        }
        
        $this->viptypeCache = $vipdata;
    ...
    public function index() {
        ...
        if ($search) {
            ...
            //会员组
            $groupid = input('get.groupid', 0, 'intval');
            if ($groupid > 0) {
                $where['groupid'] = array("EQ", $groupid);
            }
            //vip类型
            $vip = input('get.vip', 0, 'intval');
            if ($vip > 0) {
                $where['vip'] = array("EQ", $vip);
            }
    ...
    public function add() {
        if (IS_POST) {
            ...
            $this->assign('groupCache', $groupCache);
            $this->assign('groupsModel', $groupsModel);
            $this->assign("viptype",  $this->viptypeCache);
            $this->display();
        }
    }
    ...
    public function edit() {
        if (IS_POST) {
            ...
            $userid = input('get.userid', 0, 'intval');
            $modelid = input('get.modelid', 0, 'intval');
            //主表
    ...
    public function connect() {
        $db =  model("member/Connect");
        if (IS_POST) {
            //批量删除
            $connectid = input('post.connectid');
            ...
            $connectid = input('get.connectid', 0, 'intval');
            if ($connectid) {
                //单个删除
                if ($db->connectDel($connectid)) {
                    $this->success("取消绑定成功！");
                ...
                foreach ($data as $k => $r) {
                    $data[$k]['username'] = $this->member->where(array("userid" => $r['uid']))->column("username");
                    $data[$k]['userid'] = $r['uid'];
                }
                $this->assign("Page", $page->show('Admin'));
                $this->assign("data", $data);
