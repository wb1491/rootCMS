...
class Memberbase extends Base {
    ...
    protected function _initialize() {
        parent::_initialize();
        $this->memberConfig = sys_cache("Member_Config");
        $this->memberGroup = sys_cache("Member_group");
        $this->memberModel = sys_cache("Model_Member");
        $this->memberDb =  model('Member');
        ...
        if ($this->userid) {
            ...
            if ($this->isConnectLogin) {
                ...
                if (MODULE_NAME == 'Member' && in_array(CONTROLLER_NAME, array('Index', 'Feed')) && in_array(ACTION_NAME, array('index', 'home', 'logout', 'fetchfeed'))) {
                    ...
                    $this->error('请完善资料后在继续操作！<a href="' .  url("member/Publics/connectregister") . '">Go...马上去完善资料？</a>',  url("member/Publics/connectregister"));
                }
            }
            service("Passport")->logoutLocal();
    ...
    protected function message($message = '', $data = array(), $type = 'success') {
        ...
        if (IS_AJAX) {
            ...
            if (empty($callback)) {
                ...
                $type = 'JSONP';
                config('VAR_JSONP_HANDLER', 'callback');
            }
			$data['info'] = $message;
            ...
            if ('success' == $type || $type == true) {
                $this->success($message, url("member/User/profile"));
            } else {
                $this->error($message);
            }
        }
