...
class Publics extends Memberbase {
    ...
    public function doLogin() {
        ...
        $escape = input('request.escape', 0, 'intval');
        //用户名
        $loginName = input('request.loginName', null, 'trim');
        //密码
        $password = input('request.password');
        //下次自动登陆
        $cookieTime = input('request.cookieTime', 0, 'intval');
        //验证码
        $vCode = input('request.vCode');
        if ($escape) {
...
//            //================================================================
//            //还未购买vip的到购买vip页面
//            if($userInfo['vip']<=0 || $userInfo['overduedate'] < time()){
//                redirect( url("Publics/getVip"));
//            }
//            //================================================================
            
            //注册在线状态
             model('Online')->registerOnlineStatus($userid);
            //tag 行为点
...
//        
        ...
        //手机验证码
        $mCode = input("post.mCode");
        $bcode = session("mobile".session_id());
        if($mCode != $bcode){
            $this->error("手机验证码错误！");
        }
        ...
        if ($info) {
            ...
            if ($userid > 0) {
                ...
                if (false !== $this->memberDb->where(array('userid' => $memberinfo['userid']))->save($info)) {
                    if ($this->memberConfig['enablemailcheck'] && !empty($info['email']) ) {
                        //发送邮件
                        ...
                        $this->success("邮件已经发送到你注册邮箱，根据邮件内容完成验证操作！",  url('member/Index/index'));
                        exit;
                    } else {
                        if (!$info['checked']) {
                            $this->success("会员注册成功，但需要管理员审核通过！",  url('member/Index/index'));
                            exit;
                        }
                    }
                    //注册登陆状态
    ...
    public function checkNickname() {
        ...
        if ($this->memberDb->where(array('nickname' => $nickname))->column('nickname')) {
            $this->error('该昵称已经存在！');
        }
    ...
    public function connectbinding() {
        ...
        $access_token = session('access_token');
        //登陆用户名
        $loginName = input('post.loginName', '', 'trim');
        //登陆密码
        $password = input('post.password', '', 'trim');
        ...
        if ($Connect->connectAdd($data)) {
            ...
            $this->success('帐号绑定成功！',  url("member/Index/index"));
        } else {
