...
class ModelController extends AdminBase {
    ...
    protected function _initialize() {
        parent::_initialize();
        $this->memberModel =  model('Content/Model');
    }
        ...
        if (IS_POST) {
            ...
            if ($data) {
                ...
                if ($modelid) {
                    ...
                    //更新缓存
                     model('member/Member')->member_cache();
    ...
    public function move() {
        if (IS_POST) {
            ...
            //目标模型
            $to_modelid = input('post.to_modelid', 0, 'intval');
            if (empty($to_modelid)) {
                $this->error("请选择目标模型！");
            }
            ...
            if (false !== $member->where(array("modelid" => $modelid, 'type' => 2))->save(array("modelid" => $to_modelid))) {
                $this->success("会员转移成功！",  url("Model/index"));
            } else {
                $this->error("会员转移失败！",  url("Model/index"));
            }
        } else {
            $modelid = input('get.modelid', 0, 'intval');
            $model = cache("Model_Member");
