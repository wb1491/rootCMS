...
class ConnectsinaController extends Base {
    ...
    public function index() {
        //跳转
        header("location:" .  model("member/Connect")->getUrlConnectSinaWeibo());
    }
    ...
    protected function user($Result) {
        $openid = $Result['uid'];
        if (!$openid) {
            $this->error("授权失败！",  url("Connectsina/index"));
        }
        ...
        if ($uid) {
            ...
            if ($info) {
                //待审核
                if ($info['checked'] == 0) {
                    $this->error("该帐号还未审核通过，暂无法登陆！",  url("member/Index/login"));
                }
                if (service("Passport")->registerLogin($info)) {
                    $forward = $_REQUEST['forward'] ? $_REQUEST['forward'] : cookie("forward");
                    redirect($forward ? $forward :  url("member/Index/index"), 0, '');
                } else {
                    $this->error("登陆失败！",  url("member/Index/login"));
                }
            } else if ($status == -1) {
                $this->error("用户不存在！",  url("member/Publics/connectregister"));
            } else {
                $this->error("登陆失败！",  url("member/Index/login"));
            }
        } else {
            ...
            //不存在，跳转到注册页面
            $this->redirect('member/Publics/connectregister');
        }
    }
