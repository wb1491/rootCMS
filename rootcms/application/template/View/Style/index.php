...
<div class="wrap J_check_wrap">
  ...
  <table width="100%" cellspacing="0">
        ...
        <tbody>
          ...
          <tr>
            <td align="left" colspan="3"><a href="{:url("Template/Style/index",array("dir"=>urlencode(  str_replace(basename($dir).'-','',str_replace('/','-',$dir))   )     )   )}"><img src="{$config_siteurl}statics/images/folder-closed.gif" />上一层目录</a></td>
          </tr>
          </if>
          <volist name="tplist" id="vo">
          <tr>
            <td align="left">
            <if condition=" '.'.fileext(basename($vo)) == config('TMPL_TEMPLATE_SUFFIX')">
            <img src="{$tplextlist[$vo]}" />
            <a href="{:url("Template/Style/edit",array("dir"=>urlencode(str_replace('/','-',$dir)),"file"=>basename($vo)))}"><b>{$vo|basename}</b></a></td>
            <td align="center"> 
            <?php
			$op = array();
            ...
            <else />
            <img src="{$tplextlist[$vo]}" />
            <a href="{:url("Template/Style/index",array("dir"=>urlencode(str_replace('/','-',$dir).basename($vo).'-') ))}"><b>{$vo|basename}</b></a></td>
            <td></td>
            </if>
          </tr>
          </volist>
