...
class StyleController extends AdminBase {
    ...
    public function add() {
        if (IS_POST) {
            ...
            $file = $file['filename'] . config("TMPL_TEMPLATE_SUFFIX");
            //模板内容
            $content = \Input::getVar(input('post.content', '', ''));
            //目录
            $dir = TEMPLATE_PATH . input('post.dir', '', '');
            ...
            if ($status) {
                $this->success("保存成功！",  url("Template/Style/index"));
            } else {
