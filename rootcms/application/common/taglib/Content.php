...
class Content extends TagLib {
    ...
    protected $tags = [
        //后台模板标签
        'admintemplate' => ['attr' => 'file', 'close' => 0],
        //前台模板标签
        'template'      => ['attr' => 'file,theme', 'close' => 0],
        //区块缓存
        'blockcache'    => ['attr' => 'cache', 'close' => 1],
        //上一篇
        'previous'      => ['attr' => 'catid,id,target,msg,field', 'close' => 0],
        //下一篇
        'next'          => ['attr' => 'catid,id,target,msg,field', 'close' => 0],
        //导航标签
        'navigate'      => ['attr' => 'cache,catid,space,blank', 'close' => 0],
        //Form标签
        'forms'          => ["attr" => "function,parameter", "close" => 0],
        //SQL标签
        'get'           => ["attr" => 'sql,cache,page,dbsource,return,num,pagetp,pagefun,table,order,where', 'level' => 3],
        //推荐位标签
        'position'      => ['attr' => 'action,cache,num,return,posid,catid,thumb,order,where', 'level' => 3],
        //评论标签
        'comment'       => ['attr' => 'action,cache,num,return,catid,hot,date', 'level' => 3],
        //Tags标签
        'tags'          => ['attr' => 'action,cache,num,page,pagetp,pagefun,return,order,tag,tagid,where', 'level' => 3],
        //sp模块调用标签
        'spf'           => ['attr' => 'module,action,cache,num,page,pagetp,pagefun,return,where,order', 'level' => 3],
        //内容标签
        'content'       => ['attr' => 'action,cache,num,page,pagetp,pagefun,return,where,moreinfo,thumb,order,day,catid,output', 'level' => 3],
    ];
     ...
     */
    public function tagAdmintemplate($attr, $content) {
        $config = sys_cache('Config');
        $theme = isset($attr['theme']) && !empty($attr['theme']) ? $attr['theme'] : $config['admintheme'];
        $theme = !empty($theme) ? $theme : "default";
        
        $theme = "admin" . DS . $theme;
        
        $templateFile = str_replace(["/",":"],DS,$attr['file']);
        //不是直接指定模板路径的
        if (false === strpos($templateFile, config('template.view_suffix'))) {
            $templateFile = TEMPLATE_PATH . $theme . DS . $templateFile . "." . config('template.view_suffix');
        } else {
            $templateFile = TEMPLATE_PATH . $theme . DS . $templateFile;
        }
        //判断模板是否存在
        if (!file_exists($templateFile)) {
            $templateFile = str_replace($theme . DS , 'default' . DS, $templateFile);
    ...
    public function tagTemplate($attr, $content) {
        $config = sys_cache('Config');
        $theme = isset($attr['theme']) && !empty($attr['theme']) ? $attr['theme'] : $config['theme'];
        
        $theme = "index" . DS . $theme;
     ...
     * @return type
     */
    public function tagBlockcache($tag, $content) {
        $cacheIterateId = to_guid_string(array($tag, $content));
        //缓存时间
        $cache = (int) $tag['cache'] ? : 300;
        $parsestr = '<?php ';
        $parsestr .= ' $_cache = cache("' . $cacheIterateId . '"); ';
        $parsestr .= ' if ($_cache) { ';
        ...
        $parsestr .= ' if ($_html) { cache("' . $cacheIterateId . '", $_html, ' . $cache . ');}';
        $parsestr .= ' echo $_html; ';
        $parsestr .= ' } ';
        $parsestr .= ' ?>';
        return $parsestr;
     ...
     * @return type
     */
    public function tagPrevious($tag, $content) {
        //当没有内容时的提示语
        $msg = !empty($tag['msg']) ? $tag['msg'] : '已经没有了';
        //是否新窗口打开
        $target = !empty($tag['blank']) ? ' target=\"_blank\" ' : '';
        //返回对应字段内容
        $field = $tag['field'] && in_array($tag['field'], ['id', 'title', 'url']) ? $tag['field'] : '';
        if (!$tag['catid']) {
            $tag['catid'] = '$catid';
        }
        if (!$tag['id']) {
     ...
     * @return array|string
     */
    public function tagNavigate($tag, $content) {
     ...
     * 标签：<forms/>
     * 作用：生成各种表单元素
     * 用法示例：<forms function="date" parameter="name,$valeu"/>
     * 参数说明：
    ...
    public function tagContent($tag, $content) {
        ...
        //方法
        $tag['action'] = $action = isset($tag['action']) ? trim($tag['action']):'';
        //sql语句的where部分
    ...
    public function tagComment($tag, $content) {
        ...
        $parseStr = '<?php';
        $parseStr .= ' $comment_tag = new \app\comments\taglib\Comments();';
    ...
    public function tagTags($tag, $content) {
        ...
        $parseStr = '<?php';
        $parseStr .= ' $Tags_tag = new \app\content\taglib\Tags();';
        //如果有传入$page参数，则启用分页。
        if ($page && in_array($action, ['lists'])) {
            //分页配置处理
            $pageConfig = $this->resolvePageParameter($tag);
    ...
    public function tagGet($tag, $content) {
        ...
        $tag['pagefun'] = $pagefun = empty($tag['pagefun']) ? "page" : trim($tag['pagefun']);
        //缓存时间
        $tag['cache'] = $cache = isset($tag['cache'])?(int) $tag['cache']:600;
        //每页显示总数
        ...
        $tag['sql'] = $sql = str_replace(array("think_", "cms_"), config("database.prefix"), strtolower($tag['sql']));
        //数据源
        $tag['dbsource'] = $dbsource = isset($tag['dbsource'])?$tag['dbsource']:'';
        //表名
        ...
        if ($page) {
            if ($table) {
                ...
                $parseStr .= ' }else{ ';
                $parseStr .= ' $get_db = db(ucwords("' . $table . '"));';
                //如果定义了数据源 
                if ($dbsource) {
//                    $dbSource = F('dbSource');
...
//                    $parseStr .= ' $get_db->db(1,"' . $db . '"); ';
                ...
                if ($_sql = preg_replace('/select([^from].*)from/i', "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                    ...
                    if (substr(trim($sql), 0, 1) == '$') {
                        $parseStr .= ' $sql = str_replace(array("think_", "shuipfcms_"), config("DB_PREFIX"),' . $sql . ');';
...
//                        $parseStr .= ' $get_db->db(1,"' . $db . '"); ';
            ...
            //缓存处理
            $parseStr .= '      if($cache){ cache($cacheID ,array("count"=>$count,"data"=>$' . $return . '),$cache); }; ';
            $parseStr .= ' } ';
        } else {
            $parseStr .= ' $cache = ' . $cache . ';';
            if ($table) {
                ...
                //判断是否变量传递
                if (substr(trim($sql), 0, 1) == '$') {
                    $parseStr .= ' $_sql = str_replace(array("think_", "shuipfcms_"), config("DB_PREFIX"),' . $sql . ');';
                } else {
                    $parseStr .= ' $_sql = "' . str_replace('"', '\"', $sql) . '";';
                }
                $parseStr .= ' $get_db = db();';
     ...
     * @return array
     */
    public function tagSpf($tag, $content) {
        //模块
        ...
        //拼接php代码
        $parseStr = '<?php';
        $parseStr .= '  $' . $mo . 'TagLib = new \\app\\' . $mo . '\\TagLib\\' . $mo . 'TagLib(); ';
        //如果有传入$page参数，则启用分页。
        if ($page) {
            //分页配置处理
            $pageConfig = $this->resolvePageParameter($tag);
