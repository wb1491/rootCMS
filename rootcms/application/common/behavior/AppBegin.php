...
class AppBegin {
    ...
    public function run(&$param) {
        ...
        //禁止访问
        $this->prohibitAccess($param['module'][0]);
        //模块(应用)静态资源目录地址extresdir
        //define('MODULE_EXTRESDIR', 'statics'.DS.'extres'. DS . strtolower(MODULE_NAME) . DS);
        //define('MODULE_EXTRESDIR', 'statics'.DS.'extres'. strtolower(MODULE_NAME) . DS );
    }

    /**
     * 禁止非法访问
     */
    private function prohibitAccess($modulename) {
        if(empty($modulename) && !empty(MODULE_NAME)){
            $modulename = MODULE_NAME;
        }
        if (!in_array($modulename, config('MODULE_ALLOW_LIST'))) {
            if (APP_DEBUG) {
        ...
        if ($modulename == 'admin' && isModuleInstall('domains') && $config['domainaccess']) {
