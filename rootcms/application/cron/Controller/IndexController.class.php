...
class IndexController extends ShuipFCMS {
    ...
    protected function _initialize() {
        parent::_initialize();
        //单个任务最大执行时间
        $CRON_MAX_TIME = config('CRON_MAX_TIME');
        if (empty($CRON_MAX_TIME)) {
            config('CRON_MAX_TIME', 3000);
        }
    }
    ...
    public function index() {
        ...
        if (is_writable($lockfile) && filemtime($lockfile) > $_SERVER['REQUEST_TIME'] - config('CRON_MAX_TIME')) {
            //return;
        } else {
            //设置指定文件的访问和修改时间
            touch($lockfile);
    ...
    private function runCron() {
        $_time = time();
        $cron =  model("Cron/Cron")->where(array("isopen" => array("EGT", 1)))->order(array("next_time" => "ASC"))->find();
        //检测是否还有需要执行的任务
        ...
        //获取下一次执行时间
        $nexttime =  model("Cron/Cron")->getNextTime($cron['loop_type'], $day, $hour, $minute);
        //更新计划任务的下次执行时间
         model("Cron/Cron")->where(array("cron_id" => $cron['cron_id']))->save(array(
            "modified_time" => $_time,
            "next_time" => $nexttime,
        ));
