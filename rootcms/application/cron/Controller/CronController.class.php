    ...
    public function add() {
        if (IS_POST) {
            if ($this->db->CronAdd($_POST)) {
                $this->success("计划任务添加成功！",  url("Cron/index"));
            } else {
            ...
            $db = M('Customtemp');
            $array = $db->order(array("tempid" => "DESC"))->column("tempid,name");
    ...
    public function edit() {
        if (IS_POST) {
            if ($this->db->CronEdit($_POST)) {
                $this->success("修改成功！",  url("Cron/index"));
            } else {
                $this->error($this->db->getError());
            }
        } else {
            $cron_id = input('get.cron_id', 0, 'intval');
