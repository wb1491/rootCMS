...
class Behavior extends \think\Model {
    ...
    public function editBehavior($data) {
        ...
        if ($data) {
            if (false !== $this->where(array('id' => $behaviorId, 'system' => 0))->save($data)) {
                $behaviorRule = model('BehaviorRule');
                //查询出原本所属该行为的全部规则id
                $oldRuleId = $behaviorRule->where(array('behaviorid' => $behaviorId))->column('ruleid', true);
                //更新旧的规则
                if (!empty($ruleList)) {
    ...
    protected function parseBehavior($action = null) {
        ...
        //查询规则
        $behaviorRule = db('BehaviorRule');
        $ruleList = $behaviorRule->where('behaviorid',$info['id'])->order(['listorder' => 'ASC', 'ruleid' => 'ASC'])->column('ruleid','rule','system','module');
        if (empty($ruleList)) {
            return false;
        }
    ...
    protected function executionSQL($rule = false, &$params = null) {
        ...
        //SQL语句
        $sql = str_replace(array("think_", "shuipfcms_"), config("DB_PREFIX"), $rule['sql']);
        //参数处理
        if (!empty($sql)) {
