...
<div class="wrap jj">
  <div class="nav">
    <ul class="cc">
      <li class="current"><a href="{:url('Adminmanage/chanpass')}">修改密码</a></li>
    </ul>
  </div>
  <!--====================用户编辑开始====================-->
  <form class="J_ajaxForm" action="{:url('admin/Adminmanage/chanpass')}" method="post">
...
$(function () {
    Wind.use('ajaxForm', function () {
		$('input[name=password]').on("focusout",function(e){
			...
			$.get("{:url('admin/Adminmanage/public_verifypass')}", { password: ""+passwords+"" } ,function(data){
				if(data.state == "fail"){
