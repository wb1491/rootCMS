...
class Passport extends \system\Service {
    ...
    public static function connect($name = '', $options = array()) {
        ...
        //附件存储方案
        $class = strpos($type, '\\') ? $type : '\driver\\Passport\\' . ucwords(strtolower($type));
        if (class_exists($class)) {
            $connect = new $class($options);
        } else {
            exception("通行证驱动 {$class} 不存在！");
        }
        return $connect;
    }
    ...
    public function getCookieUid() {
        $userId = \util\Encrypt::authcode(cookie(self::userUidKey), 'DECODE');
     ...
     * 用户余额变更
     * @param type $uid 数字为用户ID，其他为用户名
     * @param type $integral 正数增加余额，负数扣除余额
     * @return int 成功返回当前积分数，失败返回false，-1 表示当前余额不够扣除
     */
    public function userAmount($uid, $integral) {
        if (!isModuleInstall('Member')) {
            return true;
        }
        $map = array();
        ...
        $member = M('Member');
        $tinfo = $member->where($map)->select();
        if (empty($tinfo)) {
            return false;
        }
        $info = $tinfo[0];
        $point = $info['amount'] + $integral;
        if ($point < 0) {
            $point = 0;
        }
        $info['amount'] = $point;
        //更新
        if (false !== $member->save($info)) {
            return true;
        }
        return false;
    }
     ...
     * 升级vip
     * @param type $uid 用户id
     * @param type $vipid 用户vip等级id
     * @param type $amount vip等级费用
     */
    public function updateVip($uid,$vipid,$amount){
        if (is_numeric($uid)) {
            $map['userid'] = $uid;
        } else {
            $map['username'] = $uid;
        ...
        if(!empty($vipid)){
            ...
            $vipInfo = getVipRankByAmount($amount);
            $vipid = $vipInfo['id'];
        }
        if(empty($vipInfo)){
            $this->error ="不存在的VIP等级";
            return false;
        }
    ...
    public function registerLogin(array $user, $is_remeber_me = 604800) {
        $key = \Util\Encrypt::authcode((int) $user['userid'], '');
    ...
    public function getAvatarPath($uid) {
        ...
        return config("UPLOADPATH").'avatar/' . $dir1 . '/' . $dir2 . '/' . $dir3 . '/';
    }
    /**
     * 根据用户名及头像扩展名获取头像文件名称
     * @param type $uid
     ...
     * @return type
     */
    public function getAvatarFile($uid,$fileext="jpg"){
        return $uid . "_180x180.".$fileext;
    }
