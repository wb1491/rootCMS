...
class Attachment extends \system\Service {
    ...
    public static function connect($name = '', $options = array()) {
        ...
        //附件存储方案
        $class = strpos($type, '\\') ? $type : '\Driver\\Attachment\\' . ucwords(strtolower($type));
        if (class_exists($class)) {
            $connect = new $class($options);
        } else {
            exception("附件驱动 {$class} 不存在！");
        }
        return $connect;
    }
        ...
        if ($isurl == 2 || empty($content)) {
            ...
            if (is_array($att_arrs) && !empty($att_arrs)) {
                foreach ($att_arrs as $r) {
                    $aid = M("Attachment")->where(array('authcode' => $r))->column("aid");
                    if ($aid) {
