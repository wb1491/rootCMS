...
class RBAC {
    ...
    static public function authenticate($map = '') {
        ...
        //查询是否有权限
        return  model('admin/Access')->isCompetence($map);
    }
    ...
    static function saveAccessList($authId = null) {
        ...
        // 对管理员开发所有权限
        if (config('USER_AUTH_TYPE') != 2 && User::getInstance()->isAdministrator($authId) !== true)
            session("ACCESS_LIST", RBAC::getAccessList($authId));
        return;
    }
    ...
    static function checkAccess() {
        ...
        if (config('USER_AUTH_ON')) {
            ...
            if ("" != config('REQUIRE_AUTH_MODULE')) {
                //需要认证的模块
                $_controller['yes'] = explode(',', strtoupper(config('REQUIRE_AUTH_MODULE')));
            } else {
                //无需认证的模块
                $_controller['no'] = explode(',', strtoupper(config('NOT_AUTH_MODULE')));
            }
            //检查当前模块是否需要认证
    ...
    static public function getAccessList($authId) {
        ...
        //检查角色
        $roleinfo =  model('admin/Role')->where(array('id' => $role_id))->find();
        if (empty($roleinfo) || empty($roleinfo['status'])) {
            return false;
        }
        //该角色全部权限
        $access =  model('admin/Access')->getAccessList($role_id);
        $accessList = array();
