...
class Module {
    ...
    static public function getInstance() {
        return Loader::import('\\libs\\system\\Module');
    }
    ...
    public function install($moduleName = '') {
        ...
        if (!empty($config['tags'])) {
             model('Common/Behavior')->moduleBehaviorInstallation($moduleName, $config['tags']);
        }
        //安装结束，最后调用安装脚本完成
    ...
    public function uninstall($moduleName = '') {
        ...
        //去除对应行为规则
         model('Common/Behavior')->moduleBehaviorUninstall($moduleName);
        //注销缓存
    ...
    private function installMenu($moduleName = '', $file = 'Menu') {
        ...
        $status =  model('admin/Menu')->installModuleMenu($menu, $this->config($moduleName));
        if ($status === true) {
            return true;
        } else {
    ...
    private function runSQL($moduleName = '', $Dir = 'Install') {
        ...
        $sql = $this->resolveSQL($sql, config("DB_PREFIX"));
