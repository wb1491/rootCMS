...
class Content extends Components {
    ...
    public function data($data = '') {
        ...
        if (is_object($data)) {
            ...
            exception(L('_DATA_TYPE_INVALID_'));
        }
        $this->data = $data;
        return $this;
    }
    ...
    public function add($data = '') {
        ...
        if (!defined('IN_ADMIN') || (defined('IN_ADMIN') && IN_ADMIN == false)) {
            ...
            //添加用户名
            $data['username'] = \app\admin\service\User::getInstance()->username;
            $data['sysadd'] = 1;
        }
    ...
    public function edit($data = '') {
        ...
        $model = ContentModel::getInstance($this->modelid);
        //真实发布时间
        $data['inputtime'] = $inputtime = $model->where(array("id" => $this->id))->column('inputtime');
        //更新时间处理
        if ($data['updatetime'] && !is_numeric($data['updatetime'])) {
            $data['updatetime'] = strtotime($data['updatetime']);
        } elseif (!$data['updatetime']) {
        ...
        if ($data) {
            if ($model->where(array('id' => $id, 'catid' => $catid))->save(array('status' => $status)) !== false) {
                ...
                if ($data['sysadd'] == 0 && $status == 99 && isModuleInstall('Member')) {
                    //检查是否已经赠送过积分
                    $integral = M('MemberContent')->where(array('content_id' => $id, 'catid' => $catid))->column('integral');
                    if (!$integral) {
                ...
                if ($content_ishtml && $status != 99) {
                    ...
                    $this->data($data)->deleteHtml();
                    //删除tags
                     model('Content/Tags')->deleteAll($data['id'], $data['catid'], $this->modelid);
                } elseif ($status == 99) {
                    //更新tags
    ...
    public function delete($id = '', $catid = '') {
        ...
        //删除推荐位的信息
        if (!empty($data['posid'])) {
             model('Content/PositionData')->deleteByModeId($this->modelid, $id);
        }
        //标签
    ...
    protected function getThumb(&$data) {
        ...
        if (empty($data['thumb'])) {
            ...
            $auto_thumb_no = input('.auto_thumb_no', 1, 'intval') - 1;
