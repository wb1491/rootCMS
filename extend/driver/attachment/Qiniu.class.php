...
namespace driver\attachment;

use service\Attachment;

class Qiniu extends Attachment {

    protected $addonConfig = array();
    ...
    function __construct($options = array()) {
        ...
        //附件存放路径
        $this->options['uploadfilepath'] = config('UPLOADFILEPATH');
        //允许上传的附件大小
        ...
        //上传目录
        $this->options['savePath'] =  model('Attachment/Attachment')->getFilePath($this->options['module'], $this->options['dateFormat'], $this->options['time']);
        //如果生成缩略图是否移除原图
        ...
        $QiniuClouds = cache('Addons.QiniuClouds');
        $this->addonConfig = $QiniuClouds['config'];
        require_cache(PROJECT_PATH . '/Addon/QiniuClouds/Qiniu/Client.php');
        \Qiniu\Client::registerAutoloader();
        $config = array(
            'access_key' => $this->addonConfig['qiniuak'],
            'secret_key' => $this->addonConfig['qiniusk'],
        );
        $this->handler = new \Qiniu\Client($config);
    }
    ...
    public function FTPuplode($upfile, $file) {
        ...
        // 远程存放地址
        $remote = str_replace(SITE_PATH, $this->addonConfig['qiniuuppat'], $file);
        ...
        $status = $this->handler->putFile($this->addonConfig['qiniubucket'], $remote, $body);
        if (isset($status[0]['key']) && $status[0]['key']) {
            return true;
        }
        $this->error = '附件上传失败！';
        return false;
    ...
    public function upload($Callback = false) {
        ...
        if ($this->handlerLocal->upload($Callback)) {
            ...
            foreach ($info as $i => $value) {
                ...
                $aid =  model('Attachment/Attachment')->fileInfoAdd($value, $this->options['module'], $this->options['catid'], $this->options['thumb'], $this->options['isadmin'], $this->options['userid'], $this->options['time']);
                if ($aid) {
    ...
    public function delFile($file) {
        //是否为数字
        if (is_int($file)) {
            $info =  model('Attachment')->where(array("aid" => $file))->find();
            if ($info) {
                //附件地址
                $filepath = $this->addonConfig['qiniuuppat'] . str_replace(SITE_PATH, '', $this->options['uploadfilepath'] . $info['filepath']);
                if (! model('Attachment/Attachment')->where(array("aid" => $file))->delete()) {
                ...
                try {
                    ...
                    return $this->handler->rsDelete($this->addonConfig['qiniubucket'], $filepath) ? false : true;
                } catch (Exception $exc) {
            ...
            $info =  model('Attachment/Attachment')->where(array("authcode" => $authcode))->find();
            if ($info) {
                //附件地址
                $filepath = $this->addonConfig['qiniuuppat'] . str_replace(SITE_PATH, '', $this->options['uploadfilepath'] . $info['filepath']);
                if ( model('Attachment/Attachment')->where(array("authcode" => $authcode))->delete()) {
                    try {
                        //FTP删除
                        if (substr($filepath, 0, 1) == '/') {
                            $filepath = substr($filepath, 1);
                ...
                try {
                    ...
                    //FTP删除
                    $filepath = $uploadfilepath . $newFile;
                    if (substr($filepath, 0, 1) == '/') {
                        $filepath = substr($filepath, 1);
                    }
                    return $this->handler->rsDelete($this->addonConfig['qiniubucket'], $filepath) ? false : true;
    ...
    public function download($value, $watermark = null, $ext = 'gif|jpg|jpeg|bmp|png') {
        ...
        foreach ($remotefileurls as $k => $file) {
            ...
            if ($status) {
                ...
                $aid =  model('Attachment/Attachment')->fileInfoAdd($info, $this->options['module'], $this->options['catid'], $this->options['thumb'], $this->options['isadmin'], $this->options['userid'], $this->options['time']);
                //上传到FTP
